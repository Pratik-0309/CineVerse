          ...
          <div className="flex-1 space-y-6 pb-4">
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm font-bold text-gray-400">
                <span className="flex items-center gap-1 text-yellow-400"><Star size={16} fill=""/> {movie.vote_average?.toFixed(1)}</span>
                <span>•</span>
                <span className="flex items-center gap-1"><Clock size={16}/>{formatDuration(movie.runtime)}</span>
                <span>•</span>
            ...
            <div className="pt-8 max-w-3xl">
              <h3 className="text-white text-xl font-black uppercase tracking-widest mb-3">Storyline</h3>
            ...
            <div className="flex flex-wrap gap-2 pt-4">
              {movie.genres?.map(genre => (
                <span key={genre.id} className="px-4 py-1 bg-zinc-900 border border-white/10 rounded-full text-xs font-bold text-gray-400 uppercase tracking-tighter">
                  {genre.name}
                </span>
              ))}
            </div>
